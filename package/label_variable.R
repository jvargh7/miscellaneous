...
retrieve_label <- function(vec, var_name,
  ...
  if(is.null(var_labels)){
    var_labels_file <- "variable_list.xlsx"
    print(paste0("Using default if no existing variable labels: ",var_labels_file))
    
    var_labels <- readxl::read_excel(paste0(path_support_repo,
                                                "/documentation/",var_labels_file),
...
label_variables <- function(df,domain_name = NULL,var_labels=NULL,...){
